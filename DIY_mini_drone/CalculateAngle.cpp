...
Struct_Angle Angle = {0,0,0,0,0,0,0,0,0};

void CalculateAccAngle(Struct_Angle* Angle, Struct_MPU6050* MPU6050)
{
  Angle->acc_roll  = atanf(-MPU6050->acc_x / sqrt(powf(MPU6050->acc_y,2) + powf(MPU6050->acc_z,2))) * RADIAN_TO_DEGREE;
  Angle->acc_pitch = atanf(MPU6050->acc_y / sqrt(powf(MPU6050->acc_x,2) + powf(MPU6050->acc_z,2))) * RADIAN_TO_DEGREE;
...
static float dt = 0.002f; //Sample rate is 500Hz
